...
X_Exp=numpy.array([trainDB['x1'].values,trainDB['T/K'].values]).T
...
errY_Exp2=numpy.array([trainDB['errVisc'].values]).T *1e4
...
# Profiles
fig, axs = plt.subplots(2, 1, figsize=(3, 4), dpi=300)

axs[1].plot(X_Train[:,0], Y_Train1, 'o', color='black', markersize=10, markerfacecolor='none')
axs[0].plot(X_Train[:,0], Y_Train2, 'o', color='black', markersize=10, markerfacecolor='none')

dy1 = [0.5, -0.5, 0.3, 0.5, -0.5]
dx1 = [0, 0, -0.05, 0, 0]
dy2 = [-0.7, 0.7, 0.0, -0.7, 0.7]
dx2 = [0, 0, -0.05, 0, 0]
for i, (x, y1, y2) in enumerate(zip(X_Train[2:,0], Y_Train1[2:], Y_Train2[2:]), start=1):
...
axs[1].errorbar(X_s[0][:,0], numpy.squeeze(Y_s1[0]), numpy.squeeze(errY_s1[0]), fmt='o', color='black', label='278 K')
axs[1].errorbar(X_s[1][:,0], numpy.squeeze(Y_s1[1]), numpy.squeeze(errY_s1[1]), fmt='>', color='blue', label='288 K')
axs[1].errorbar(X_s[2][:,0], numpy.squeeze(Y_s1[2]), numpy.squeeze(errY_s1[2]), fmt='*', color='red', label='298 K')
axs[1].errorbar(X_s[3][:,0], numpy.squeeze(Y_s1[3]), numpy.squeeze(errY_s1[3]), fmt='D', color='purple', label='308 K')
axs[1].errorbar(X_s[4][:,0], numpy.squeeze(Y_s1[4]), numpy.squeeze(errY_s1[4]), fmt='^', color='green', label='318 K')

axs[0].errorbar(X_s[0][:,0], numpy.squeeze(Y_s2[0]), numpy.squeeze(errY_s2[0]), fmt='o', color='black')
axs[0].errorbar(X_s[1][:,0], numpy.squeeze(Y_s2[1]), numpy.squeeze(errY_s2[1]), fmt='>', color='blue')
axs[0].errorbar(X_s[2][:,0], numpy.squeeze(Y_s2[2]), numpy.squeeze(errY_s2[2]), fmt='*', color='red')
axs[0].errorbar(X_s[3][:,0], numpy.squeeze(Y_s2[3]), numpy.squeeze(errY_s2[3]), fmt='D', color='purple')
axs[0].errorbar(X_s[4][:,0], numpy.squeeze(Y_s2[4]), numpy.squeeze(errY_s2[4]), fmt='^', color='green')
...
axs[1].plot(X_s[25][:,0], Y_s1[25], color='blue')
axs[1].plot(X_s[50][:,0], Y_s1[50], color='red')
axs[1].plot(X_s[74][:,0], Y_s1[74], color='purple')
axs[1].plot(X_s[98][:,0], Y_s1[98], color='green')

axs[0].plot(X_s[0][:,0], Y_s2[0], color='black')
axs[0].plot(X_s[25][:,0], Y_s2[25], color='blue')
axs[0].plot(X_s[50][:,0], Y_s2[50], color='red')
axs[0].plot(X_s[74][:,0], Y_s2[74], color='purple')
axs[0].plot(X_s[98][:,0], Y_s2[98], color='green')
...
axs[0].legend(handles=legend_elements, ncol=3, fontsize=7, handletextpad=0.4, labelspacing=0.3, handlelength=1.2) 

axs[0].set_title(r'\textbf{MD Data}', fontsize=10)

axs[1].set_ylabel(r'$D \times 10^{-9}$ m²/s')
axs[0].set_ylabel(r'$\eta \times 10^{-4}$ Pa s')
axs[1].set_xlabel(r'$x_{tol}$')
