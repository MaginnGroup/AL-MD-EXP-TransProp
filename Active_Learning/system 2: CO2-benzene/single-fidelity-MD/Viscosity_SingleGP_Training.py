...
dbPath=r'../Databases'
# Database code
code='BENZ_SCO2_MD_30_replicates'
...
X=numpy.array([trainDB['P/bar'].values,trainDB['T/K'].values]).T
Y=numpy.array([trainDB['Visc/Pas'].values]).T *1e5
errY=numpy.array([trainDB['errVisc'].values]).T *1e5
X_Train = X.copy()
Y_Train = Y.copy()
...
pres = numpy.array([100,110,130,150,200,300])
temp = numpy.arange(303, 374, 1)
X_new = numpy.array([[P, T] for T in temp for P in pres])
...
temp1 = numpy.unique(X[:, 0])
X_s = []
Y_s = []
errY_s = []
for temp in temp1:
    indices = numpy.where(X[:, 0] == temp)[0]
    X_sub = X[indices]
    Y_sub = Y[indices]  
...
plt.errorbar(X_s[0][:,1], numpy.squeeze(Y_s[0]), numpy.squeeze(errY_s[0]), fmt='o', color='black')
plt.errorbar(X_s[1][:,1], numpy.squeeze(Y_s[1]), numpy.squeeze(errY_s[1]), fmt='>', color='blue')
plt.errorbar(X_s[2][:,1], numpy.squeeze(Y_s[2]), numpy.squeeze(errY_s[2]), fmt='*', color='red')
plt.errorbar(X_s[3][:,1], numpy.squeeze(Y_s[3]), numpy.squeeze(errY_s[3]), fmt='D', color='pink')
plt.errorbar(X_s[4][:,1], numpy.squeeze(Y_s[4]), numpy.squeeze(errY_s[4]), fmt='<', color='green')
plt.errorbar(X_s[5][:,1], numpy.squeeze(Y_s[5]), numpy.squeeze(errY_s[5]), fmt='d', color='m')

numpy.savez("data_Viscosity_MD.npz", X_s=X_s, Y_s=Y_s, errY_s=errY_s)

temp1 = numpy.unique(X_new[:, 0])
X_s = []
Y_s = []
STD_s = []
for temp in temp1:
    indices = numpy.where(X_new[:, 0] == temp)[0]
    X_sub = X_new[indices]
    Y_sub = Y_new[indices]   
...
plt.plot(X_s[0][:,1], Y_s[0], color='black')
plt.plot(X_s[1][:,1], Y_s[1], color='blue')
plt.plot(X_s[2][:,1], Y_s[2], color='red')
plt.plot(X_s[3][:,1], Y_s[3], color='pink')
plt.plot(X_s[4][:,1], Y_s[4], color='green')
plt.plot(X_s[5][:,1], Y_s[5], color='m')
plt.fill_between(X_s[0][:,1].flatten(), (Y_s[0] - 2*STD_s[0]).flatten(), (Y_s[0] + 2*STD_s[0]).flatten(), color='black', alpha=0.2)
plt.fill_between(X_s[1][:,1].flatten(), (Y_s[1] - 2*STD_s[1]).flatten(), (Y_s[1] + 2*STD_s[1]).flatten(), color='blue', alpha=0.2)
plt.fill_between(X_s[2][:,1].flatten(), (Y_s[2] - 2*STD_s[2]).flatten(), (Y_s[2] + 2*STD_s[2]).flatten(), color='red', alpha=0.2)
plt.fill_between(X_s[3][:,1].flatten(), (Y_s[3] - 2*STD_s[3]).flatten(), (Y_s[3] + 2*STD_s[3]).flatten(), color='pink', alpha=0.2)
plt.fill_between(X_s[4][:,1].flatten(), (Y_s[4] - 2*STD_s[4]).flatten(), (Y_s[4] + 2*STD_s[4]).flatten(), color='green', alpha=0.2)
plt.fill_between(X_s[5][:,1].flatten(), (Y_s[5] - 2*STD_s[5]).flatten(), (Y_s[5] + 2*STD_s[5]).flatten(), color='m', alpha=0.2)

numpy.savez("data_Viscosity_MD_gp.npz", X_s=X_s, Y_s=Y_s, STD_s=STD_s)

plt.ylabel(r'$\eta \times 10^{-5}$ Pa s')
plt.xlabel(r'$T$ (K)')
plt.xticks([300,320,340,360,380])
plt.ylim(1,11)
plt.tight_layout()
