...
errY=numpy.array([trainDB['errVisc'].values]).T *1e4
X_Train = X.copy()
Y_Train = Y.copy()
...
pres = numpy.arange(10, 700, 1)
temp = numpy.array([298,323,348,373,398,423])
...
plt.errorbar(X_s[5][:,0], numpy.squeeze(Y_s[5]), numpy.squeeze(errY_s[5]), fmt='d', color='m')
...
plt.plot(X_s[5][:,0], Y_s[5], color='m')
...
plt.fill_between(X_s[5][:,0].flatten(), (Y_s[5] - 2*STD_s[5]).flatten(), (Y_s[5] + 2*STD_s[5]).flatten(), color='m', alpha=0.2)
