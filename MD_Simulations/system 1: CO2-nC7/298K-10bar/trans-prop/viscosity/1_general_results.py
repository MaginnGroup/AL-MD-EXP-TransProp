...
convergence = np.zeros(int(Nr/2), dtype=np.float_)  
...
for i in range(Nr):
    data = np.loadtxt(f"../{i+1}/visc_time.out", delimiter = " ", skiprows=1)
...
value_error = np.std(value)/np.sqrt(Nr) #* 1.96
...
for i in range(Nr):
    plt.plot(time/1000,viscosity[:,i], color='lightsteelblue', linewidth='0.4')
plt.plot(time/1000,viscosity_mean, 'k-')
...
fig = plt.figure(figsize=(3.0, 2.5),  dpi=300)
plt.plot(time/1000,viscosity_deviation, 'k-')
plt.ylabel(r'$\sigma$'+" (Pa s)")
plt.xlabel("t (ps)")
plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
#plt.xticks([0,250,500,750,1000])
...
fig = plt.figure(figsize=(3.0, 2.5),  dpi=300)
plt.plot(np.arange(2, Nr + 1, 2), convergence*1e4, 'ko')
